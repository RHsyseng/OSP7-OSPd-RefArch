...
* Network isolation
* Ceph storage customization


==== Server Roles
OSP-d can deploy the following server roles:
...
. Service network – this network exposes the OpenStack APIs. It also handles inter-service communication between the OpenStack services and schedulers. 
. Tenant network – virtual machines communicate over this network within the cloud deployment. The addressing requirements of this network depend on the plugin that is used. 
. Storage Network – this network is dedicated for storage traffic between the Swift servers and the OpenStack servers.
