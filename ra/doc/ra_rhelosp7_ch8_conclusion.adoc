...
introduces OpenStack director -- Red Hat's intergrated management tool set 
...
3. A fully worked example of Ceph OSD and journal customization via Puppet Hiera
   data accompanied by post-installtion configuration steps for
...
case is still valid from a DevOps perpspective for staging anf
