...
+
NOTE: View
https://github.com/redhat-cip/edeploy/blob/master/docs/eDeployUserGuide.rst#appendix[Appendix
A] of the
...
compute.
+
10. Set the provisioning network nameserver. The overcloud servers
   users this nameserver for DNS resolution.
+
[subs=+quotes]
----
$ *neutron subnet-update $(neutron subnet-list | awk ' /192.0.2/ {
print $2 } ') --dns-nameserver 10.19.143.247*
$ *neutron subnet-show $(neutron subnet-list | awk ' /192.0.2/ { print
$2 } ') | grep -B 1 nameserver*
| cidr              | 192.0.2.0/24
...
http://se-sat6.syseng.bos.redhat.com --reg-org syseng
--reg-activation-key OSP-Overcloud*
